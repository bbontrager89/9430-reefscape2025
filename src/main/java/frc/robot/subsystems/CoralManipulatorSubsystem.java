...
public class CoralManipulatorSubsystem extends SubsystemBase {
  ...
  private boolean isPivotMotorActive = false;

  // ---------- Intake Motor Fields ----------
  private final SparkMax intakeMotor = new SparkMax(CoralManipulatorConstants.IntakeMotorCanId, MotorType.kBrushless);
  private double currentIntakeSpeed = 0.0;
  private boolean isIntakeMotorActive = false;
  
  // Auto-run and slowing configuration for intake
  private boolean enableAutoCurrentLimit = true;
  private double intakeAutoStopTime = Double.POSITIVE_INFINITY;
  private double intakeSlowDuration = 0.0; // Duration (in seconds) over which to decelerate
  private double intakeSlowingStep = 0.0;  // Computed once when transitioning to slowing
  
  // Timestamp when the intake motor was started (for diagnostics)
  private double intakeStartTime = Double.NEGATIVE_INFINITY;

  // Intake motor state machine
  private ManipulatorState intakeState = ManipulatorState.Stopped;

  // ---------- Dashboard and Commands ----------
  private static SendableChooser<Command> pivotCommandChooser;

  /** Creates a new CoralManipulatorSubsystem. */
  public CoralManipulatorSubsystem() {
    // Pivot motor setup
    pivotEncoder = pivotMotor.getAbsoluteEncoder();
    // Tune these PID gains as needed.
    pivotPIDController = new PIDController(1.0, 0.00, 0.0);
    pivotPIDController.setTolerance(0.005);
    
    // Intake motor setup can go here if needed.
  }
